...
function platform_independent_make() {
    ...
    change_hostfile "$path_to_hostfile" "mailhog.local"
...
function prepare_files() {
	...
	if [ ! -f .env ]; then
  		...
  		cat .env.default | sed -e "s/LOCAL_UID=.*/LOCAL_UID=$(id -u)/" | sed -e "s/GID=.*/GID=$(id -g)/" | sed -e "s/WORDPRESS_TABLE_PREFIX=.*/WORDPRESS_TABLE_PREFIX=$WORDPRESS_TABLE_PREFIX/"  > .env
